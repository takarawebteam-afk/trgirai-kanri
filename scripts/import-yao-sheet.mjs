...
const SHEET_GID = process.env.YAO_SHEET_GID || '0'
...
function getYaoYearFromPropertyNumber(propertyNumber) {
...
function normalizeYaoPostDate(value, propertyNumber) {
  ...
  const year = getYaoYearFromPropertyNumber(propertyNumber)
  if (!year) return text
...
function toYaoRecord(columns) {
  return {
    memo: normalizeText(columns[0]),
    post_date: normalizeYaoPostDate(columns[1], columns[5]),
...
async function main() {
  ...
  const { data: currentData, error: fetchError } = await supabase
    .from('sns_yao_properties')
    .select('id, property_number')

  if (fetchError) {
    throw new Error(`現在の八尾店データの読み込みに失敗しました: ${fetchError.message}`)
  }
  ...
  if (deleteIds.length > 0) {
    const { error } = await supabase.from('sns_yao_properties').delete().in('id', deleteIds)
    if (error) {
      throw new Error(`不要な八尾店データの削除に失敗しました: ${error.message}`)
    }
  }

  for (const record of updates) {
    const { id, ...payload } = record
    const { error } = await supabase.from('sns_yao_properties').update(payload).eq('id', id)
    if (error) {
      throw new Error(`八尾店データの更新に失敗しました (${record.property_number}): ${error.message}`)
    }
  }

  if (inserts.length > 0) {
    const { error } = await supabase.from('sns_yao_properties').insert(inserts)
    if (error) {
      throw new Error(`八尾店データの追加に失敗しました: ${error.message}`)
    }
  }
