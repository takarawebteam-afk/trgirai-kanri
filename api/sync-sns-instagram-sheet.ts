...
function toSheetRow(record: InstagramPropertyRecord) {
  return [
...
async function fetchAllInstagramProperties() {
  const supabase = getSupabaseClient()
  const rows: InstagramPropertyRecord[] = []
  const pageSize = 1000
  ...
  for (let from = 0; ; from += pageSize) {
    ...
    const pageRows = (data || []) as InstagramPropertyRecord[]
    rows.push(...pageRows)
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    const records = await fetchAllInstagramProperties()
    ...
    return res.status(200).json({ ok: true, count: values.length, sheetName: SHEET_NAME })
  } catch (error) {
